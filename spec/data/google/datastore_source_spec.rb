...
describe Data::Google::DatastoreSource do
  before(:each) do
    ...
    @data_store_mock = instance_double(Data::Google::DatastoreSource)
  ...
  context 'when the query method is called' do
    it 'calls query on the data_source and returns a query object' do
      expect(@data_store_mock).to receive(:query).with(@test_kind).and_return(@test_return_entity)
  ...
  context 'when the insert method is called' do
    it 'calls insert on the data_source and returns an entity object' do
      expect(@data_store_mock).to receive(:insert).with(@test_entity).and_return(@test_return_entity)

      return_value = @data_source.insert(@test_entity)
  ...
  context 'when the update method is called' do
    it 'calls update on the data_source and returns an entity object' do
      expect(@data_store_mock).to receive(:update).with(@test_entity).and_return(@test_return_entity)

      return_value = @data_source.update(@test_entity)
  ...
  context 'when the save method is called' do
    it 'calls save on the data_source and returns an entity object' do
      expect(@data_store_mock).to receive(:save).with(@test_entity).and_return(@test_return_entity)

      return_value = @data_source.save(@test_entity)
  ...
  context 'when the delete method is called' do
    it 'calls delete on the data_source and returns true' do
      expect(@data_store_mock).to receive(:delete).with(@test_id).and_return(true)

      return_value = @data_source.delete(@test_id)
